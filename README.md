...
| `data/landmarks.json` | halls, markets, schools, chapels, subdivisions, puroks from OpenStreetMap (generated by `tools/build-landmarks.py`) | re-run the script occasionally |
| `data/landmarks.extra.json` | hand-added places OpenStreetMap lacks | when a stop names a place the map cannot find |
| `data/population.json` | 2020 census population per barangay (PSA via PhilAtlas), used for litres-per-person | after the next census |
| `data/sources.json` | the citation shown beside each figure (label, exact URL, note) | when a figure's basis changes |

Every figure on the page carries its own source link where it appears. Add a key to `data/sources.json` and render it with `cite('key')` next to the number; never collect sources into a list at the bottom.
